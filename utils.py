...
t2m_freq = {'JRA55': 8, 'ERA5':24, 'MERRA2':24, 'CanESM5raw':1, 'CanESM5qdm':1, 'NorESM2raw':1, 'NorESM2qdm':1, 'MPIraw':1, 'MPIqdm':1, 'obs':1, 'CanESM5downscaled':1}
tp_freq = {'JRA55': 8, 'ERA5':24, 'MERRA2':24, 'CanESM5raw':1, 'CanESM5qdm':1, 'NorESM2raw':1, 'NorESM2qdm':1, 'MPIraw':1, 'MPIqdm':1, 'obs':1, 'CanESM5downscaled':1}

latname = {'JRA55':'latitude', 'ERA5':'latitude', 'MERRA2':'lat', 'CanESM5raw':'lat', 
           'CanESM5qdm':'lat', 'NorESM2raw':'lat', 'NorESM2qdm':'lat', 'MPIraw':'lat', 'MPIqdm':'lat', 'obs':'lat', 'CanESM5downscaled':'lat'}
lonname = {'JRA55':'longitude', 'ERA5':'longitude', 'MERRA2':'lon', 'CanESM5raw':'lon', 
           'CanESM5qdm':'lon', 'NorESM2raw':'lon', 'NorESM2qdm':'lon', 'MPIraw':'lon', 'MPIqdm':'lon', 'obs':'lon', 'CanESM5downscaled':'lon'}

precipname = {'JRA55':'tp', 'ERA5':'tp', 'MERRA2':'PRECTOTLAND', 'CanESM5raw':'pr', 'CanESM5qdm':'scen', 'NorESM2raw':'pr', 'NorESM2qdm':'scen', 'MPIraw':'pr', 'MPIqdm':'scen', 'obs':'pr', 'CanESM5downscaled':'pr'}
tempname = {'JRA55':'t2m', 'ERA5':'t2m', 'MERRA2':'T2M', 'CanESM5raw':'tas', 'CanESM5qdm':'scen', 'NorESM2raw':'tas', 'NorESM2qdm':'scen', 'MPIraw':'tas', 'MPIqdm':'scen', 'obs':'tas', 'CanESM5downscaled':'pr'}
...
def prepare_filenames(forcing, data_loc, snow_season):
        ...
        tp_JanJul = [pattern('tp', i, snow_season[1]) for i in range(1, 8)]
        
        t2m_AugDec = [pattern('t2m', i, snow_season[0]) for i in range(8, 13)]
        t2m_JanJul = [pattern('t2m', i, snow_season[1]) for i in range(1, 8)]
        
    else:
        prefix = lambda var,m,y: data_loc+var+'/'+forcing+'/'+forcing+'_'+var+'_'+str(m).zfill(2)+'_'+str(y)+'/'

        tp_AugDec = [[prefix('tp',i,snow_season[0]) + n for n in listdir(prefix('tp',i,snow_season[0])) if '.nc' in n] \
                     for i in range(8, 13)]
        tp_JanJul = [[prefix('tp',i,snow_season[1]) + n for n in listdir(prefix('tp',i,snow_season[1])) if '.nc' in n] \
                     for i in range(1,8)]

        t2m_AugDec = [[prefix('t2m',i,snow_season[0]) + n for n in listdir(prefix('t2m',i,snow_season[0])) if '.nc' in n] \
                      for i in range(8, 13)]
        t2m_JanJul = [[prefix('t2m',i,snow_season[1]) + n for n in listdir(prefix('t2m',i,snow_season[1])) if '.nc' in n] \
                      for i in range(1,8)]
...
def read_month(month, forcing, t2m_fname, tp_fname, year=None):
    
    if isin(forcing, ['MERRA2', 'CanESM5raw', 'CanESM5qdm', 'NorESM2raw', 'NorESM2qdm', 'MPIraw', 'MPIqdm', 'obs', 'CanESM5downscaled']):
...
def standardize_precip(forcing, pr_freq, t2m_freq, data):
    ...
    if forcing == 'JRA55':
        ...
        data = data / 1000 #[m/precip step] from [kg/m2/day]
        data = (pr_freq/t2m_freq) * data #[m/t2m step]
